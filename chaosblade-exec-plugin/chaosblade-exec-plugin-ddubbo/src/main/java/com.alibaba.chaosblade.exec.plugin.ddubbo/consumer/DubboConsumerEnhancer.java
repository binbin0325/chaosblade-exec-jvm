...
package com.alibaba.chaosblade.exec.plugin.ddubbo.consumer;
...
import com.alibaba.chaosblade.exec.plugin.ddubbo.DubboEnhancer;
import com.alibaba.chaosblade.exec.plugin.ddubbo.model.CallPointMatcher;
...
public class DubboConsumerEnhancer extends DubboEnhancer {
    ...
    protected void postDoBeforeAdvice(EnhancerModel enhancerModel) {
        enhancerModel.addMatcher(DDubboConstant.CONSUMER_KEY, "true");
        StackTraceElement[] stackTrace = new NullPointerException().getStackTrace();
        enhancerModel.addCustomMatcher(DDubboConstant.CALL_POINT_KEY, stackTrace, CallPointMatcher.getInstance());
    }
    ...
    protected TimeoutExecutor createTimeoutExecutor(ClassLoader classLoader, long timeout, final String className) {
        return new BaseTimeoutExecutor(classLoader, timeout) {
            ...
            public Exception generateTimeoutException(ClassLoader classLoader) {
                ...
                return new RuntimeException(DDubboConstant.TIMEOUT_EXCEPTION_MSG);
            }
        };
    }
